...
export const useGameBoard = (difficulty: Difficulty) => {
  ...
  const handleTileHover = (row: number, col: number, e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (tile.isBomb) {
      setGameOver(true);
      // Trigger fireworks
      setFireworks(fw => [...fw, { id: Date.now(), x: e.clientX, y: e.clientY }]);
    } else {
  ...
  return { board, gameOver, auraColor, fireworks, handleTileHover, handleFireworkComplete };
};
